    ...
    if (arena->alloc_position + size > arena->commit_position) {
        if (!arena->static_size) {
            ...
            if (arena->commit_position >= arena->max) {
                ...
                OS_MemoryCommit(((u8*)arena) + sizeof(M_Arena) + arena->commit_position, commit_size);
...
#define M_SCRATCH_POOL_CAP 4

typedef struct ScratchContext {
	M_Arena* scratch_pool[M_SCRATCH_POOL_CAP];
} ScratchContext;

static ScratchContext scratch_context = {0};

void M_ScratchInit(void) {
	for (u32 i = 0; i < M_SCRATCH_POOL_CAP; i++) {
		scratch_context.scratch_pool[i] = arena_make();
	}
}

void M_ScratchFree(void) {
	for (u32 i = 0; i < M_SCRATCH_POOL_CAP; i++) {
		arena_free(scratch_context.scratch_pool[i]);
	}
}
...
M_Scratch scratch_get(M_Arena** conflict_array, u32 count) {
	...
	M_Arena* scratch;
	for (u32 i = 0; i < M_SCRATCH_POOL_CAP; i++) {
		scratch = scratch_context.scratch_pool[i];
		
		b8 is_non_conflict;
		M_Arena* conflict_ptr;
		for (u32 k = 0; k < count; k++) {
			conflict_ptr = conflict_array[k];
	...
	return (M_Scratch) { result, result->commit_position };
}
...
void pool_clear(M_Pool* pool) {
	for (u8* it = (u8*)pool + sizeof(M_Pool), *preit = it;
		 it <= (u8*)pool + sizeof(M_Pool) + pool->commit_position;
		 preit = it, it += pool->element_size) {
		((M_PoolFreeNode*)preit)->next = (M_PoolFreeNode*)it;
	}
	pool->head = (M_PoolFreeNode*)((u8*)pool + sizeof(M_Pool));
}
...
void* pool_alloc(M_Pool* pool) {
	if (pool->head) {
		...
		void* commit_ptr = ((u8*)pool) + sizeof(M_Pool) + pool->commit_position;
...
void heap_clear(M_Heap* heap) {
	heap->head = ((u8*)heap) + sizeof(M_Heap);
...
void* heap_alloc(M_Heap* heap, u64 size) {
	...
	if (heap->head) {
		...
		if (delta < 0) {
			...
			if (delta >= 0 && delta < sizeof(M_HeapFreeNode)) {
				...
				M_HeapFreeNode* splitter = (u8*)ret + size;
	...
	commit_more:
	{
		...
		void* commit_ptr = ((u8*)heap) + sizeof(M_Heap) + heap->commit_position;
...
void  heap_dealloc(M_Heap* heap, void* ptr, u64 size) {
	...
	while (it) {
		if (((u8*)it) + it->size == ptr) {
			back = it;
		} else if (((u8*)ptr) + size == it) {
			front = it;
			front_prev = prev_it;
		}
